...
> **How much less creel data could be collected, on an individual survey basis while still maintaining the stratified random sampling design, and still produce harvest estimates that are NOT significantly different from harvest estimates obtained using the current creel survey design?**
...
Here I'm using harvest estimates for ceded territory lakes only.
I'm not working with a mean harvest per month or day type or anything like that because I think the metric of importance here is total harvest.
I am using harvest per acre to standardize for lake size, similar to what I did for effort because of the nature of the data. 
This isn't necessary for exploitation rate calcs for walleye because it's expressed as a rate already.
With effort and catch I used total effort and total catch regardless of species, that may or may not have been the right choice.
Here I'm using walleye harvest only, not harvest of all species. 
I think when it comes to looking at harvest numbers, what managers care about is species specific harvest not total harvest.

What I've done here is calculate harvest based on the full data sets as well as for 5 scenarios:
...
eff.25wd=eff.25wd%>%
  ...
  select(year,wbic,waterbody.name,survey.seq.no, month, daytype, species, species.code, catch, spp.effort, spp.harvest, spp.catch)%>%
...
```{r monthlyHarv,fig.width=8,fig.cap="Distribution of harvest estimates by month (panel a) for all years of CTWI creel data. Horizontal red line is overall mean harvest. Black points are outliers. Panel B is the same information presented on a log-scale. "}


# plot of harvest by month
harvestEstimates$plot.month=month(harvestEstimates$month,label = T)
p1=ggplot(harvestEstimates)+theme_classic()+
  geom_boxplot(aes(y=harvestEstimates$harvAcre, x=plot.month), fill="grey")+
  geom_hline(yintercept = mean(harvestEstimates$harvAcre, na.rm = T), color="red")+
  labs(y="Monthly Harvest per Acre", x="Month")

# plot of harvest by month - logged
p2=ggplot(harvestEstimates)+theme_classic()+
  geom_boxplot(aes(y=log(harvestEstimates$harvAcre), x=plot.month), fill="grey")+
  geom_hline(yintercept = log(mean(harvestEstimates$harvAcre)), color="red")+ # setting this manually since the 0 exp rates mess up the logging here
  labs(y="Log(Monthly Harvest per Acre)", x="Month")

ggarrange(p1,p2,nrow = 1,ncol=2,labels = 'auto')
```

```{r harvPlot, fig.cap="Distribution, on a log scale, of harvest per acre for the full data set plus 5 scenarios with reduced data. Actual = full data set, mayAug = May to August creel data only, noWinter = winter creel data removed, wd25 = 25% of weekday creel visits per month removed, wd50 = 50% of weekday creel visits per month removed, we50 = 50% of weekend creel visits per month removed."}

# creating survey-level harvest estimates for the actual data, estimate of total harvest for that creel year produced from the observed harvest data
harv.surv.a=harvestEstimates%>%
  group_by(survey.seq.no)%>%
  summarise(total.harv=sum(harvAcre,na.rm = T))
harv.surv.nw=harvestEstimates.nw%>%
  group_by(survey.seq.no)%>%
  summarise(total.harv=sum(harvAcre,na.rm = T))
harv.surv.ma=harvestEstimates.ma%>%
  group_by(survey.seq.no)%>%
  summarise(total.harv=sum(harvAcre,na.rm = T))
harv.surv.25wd=harvestEstimates.25wd%>%
  group_by(survey.seq.no)%>%
  summarise(total.harv=sum(harvAcre,na.rm = T))
harv.surv.50wd=harvestEstimates.50wd%>%
  group_by(survey.seq.no)%>%
  summarise(total.harv=sum(harvAcre,na.rm = T))
harv.surv.50we=harvestEstimates.50we%>%
  group_by(survey.seq.no)%>%
  summarise(total.harv=sum(harvAcre,na.rm = T))
...
ggplot(ttrExp)+theme_classic()+
  geom_density(aes(x=log(total.harv), fill=treat),alpha=0.2)+
  labs(x="Log(Annual Harvest per Acre)", y="Density", fill="Scenario")
...
In order to estimate parameters for a model of the harvest distribution that produced what is represented in the creel data a Bayesian modeling approach was used.
This is because the harvest data are not normally distributed and thus don't meet the assumptions of typical analyses based on normality.
...
#removing 0s since they can't be logged and if I were to make them a small number they would throw off the data and make it bimodal which would probably mean switching to a gamma distribution to model the data. There are 8 0's accounting for 1% of the data. I'm going to operate under the assumption that if a survey gives a 0 harvest estimate with the full survey, then harvs collecting less data isn't going to change that number. 
zeros.actual=sum(ttrExp$total.harv[ttrExp$treat=="actual"]==0)
zeros.nw=sum(ttrExp$total.harv[ttrExp$treat=="noWinter"]==0)
zeros.ma=sum(ttrExp$total.harv[ttrExp$treat=="mayAug"]==0)
zeros.wd25=sum(ttrExp$total.harv[ttrExp$treat=="wd25"]==0)
zeros.wd50=sum(ttrExp$total.harv[ttrExp$treat=="wd50"]==0)
zeros.we50=sum(ttrExp$total.harv[ttrExp$treat=="we50"]==0)
...
modDat=ttrExp[ttrExp$total.harv!=0,]
...
harvLL.a=function(param){
  ...
  ll=dlnorm(modDat$total.harv[modDat$treat=="actual"], meanlog = mean(log(harvs)), sdlog = sd(log(harvs)), log = T)
  return(sum(ll))
}

prior=createTruncatedNormalPrior(mean=c(mean(log(modDat$total.harv[modDat$treat=="actual"])),sd(log(modDat$total.harv[modDat$treat=="actual"]))),
                                 sd=c(1,1),
                                 lower = c(-15,0),
                                 upper = c(15,5))
...
harvLL.nw=function(param){
  ...
  ll=dlnorm(modDat$total.harv[modDat$treat=="noWinter"], meanlog = mean(log(harvs)), sdlog = sd(log(harvs)), log = T)
  return(sum(ll))
}

prior=createTruncatedNormalPrior(mean=c(mean(log(modDat$total.harv[modDat$treat=="noWinter"])),sd(log(modDat$total.harv[modDat$treat=="noWinter"]))), 
                                 sd=c(1,1),
                                 lower = c(-15,0),
                                 upper = c(15,5))
...
harvLL.ma=function(param){
  ...
  ll=dlnorm(modDat$total.harv[modDat$treat=="mayAug"], meanlog = mean(log(harvs)), sdlog = sd(log(harvs)), log = T)
  return(sum(ll))
}

prior=createTruncatedNormalPrior(mean=c(mean(log(modDat$total.harv[modDat$treat=="mayAug"])),sd(log(modDat$total.harv[modDat$treat=="mayAug"]))), 
                                 sd=c(1,1),
                                 lower = c(-15,0),
                                 upper = c(15,5))
...
harvLL.wd25=function(param){
  ...
  ll=dlnorm(modDat$total.harv[modDat$treat=="wd25"], meanlog = mean(log(harvs)), sdlog = sd(log(harvs)), log = T)
  return(sum(ll))
}

prior=createTruncatedNormalPrior(mean=c(mean(log(modDat$total.harv[modDat$treat=="wd25"])),sd(log(modDat$total.harv[modDat$treat=="wd25"]))), 
                                 sd=c(1,1),
                                 lower = c(-15,0),
                                 upper = c(15,5))
...
harvLL.wd50=function(param){
  ...
  ll=dlnorm(modDat$total.harv[modDat$treat=="wd50"], meanlog = mean(log(harvs)), sdlog = sd(log(harvs)), log = T)
  return(sum(ll))
}

prior=createTruncatedNormalPrior(mean=c(mean(log(modDat$total.harv[modDat$treat=="wd50"])),sd(log(modDat$total.harv[modDat$treat=="wd50"]))), 
                                 sd=c(1,1),
                                 lower = c(-15,0),
                                 upper = c(15,5))
...
harvLL.we50=function(param){
  ...
  ll=dlnorm(modDat$total.harv[modDat$treat=="we50"], meanlog = mean(log(harvs)), sdlog = sd(log(harvs)), log = T)
  return(sum(ll))
}

prior=createTruncatedNormalPrior(mean=c(mean(log(modDat$total.harv[modDat$treat=="we50"])),sd(log(modDat$total.harv[modDat$treat=="we50"]))), 
                                 sd=c(1,1),
                                 lower = c(-15,0),
                                 upper = c(15,5))
...
A good model fit here is evident when the distribution of the observed and predicted data overlap nearly entirely (Figure \@ref(fig:catchDataReproduction)).
...
Lastly, a Bayesian p-value was calculated for each model to further describe model fit to the data (Figure \@ref(tab:catchbpValue)).
...
Thus, with Bayesian p-values a value of 0.5 is ideal as it means the model can generate data that matches the distribution of the observed data really well (Table \@ref(tab:catchbpValue), \@ref(tab:bpValueComparison).
...
set.seed(3)
aComp=data.frame(harv.acre=c(modDat$total.harv[modDat$treat=="actual"],rlnorm(n=length(modDat$total.harv[modDat$treat=="actual"]),
                                                                         meanlog = median(pars.a[,1]),
                                                                         sdlog = median(pars.a[,2]))),
                    treat=c(rep("observed",length(modDat$total.harv[modDat$treat=="actual"])),rep("pred",length(modDat$total.harv[modDat$treat=="actual"]))))
set.seed(3)
nwComp=data.frame(harv.acre=c(modDat$total.harv[modDat$treat=="noWinter"],rlnorm(n=length(modDat$total.harv[modDat$treat=="noWinter"]),
                                                                    meanlog = median(pars.nw[,1]),
                                                                    sdlog = median(pars.nw[,2]))),
                 treat=c(rep("observed",length(modDat$total.harv[modDat$treat=="noWinter"])),rep("pred",length(modDat$total.harv[modDat$treat=="noWinter"]))))
set.seed(3)
maComp=data.frame(harv.acre=c(modDat$total.harv[modDat$treat=="mayAug"],rlnorm(n=length(modDat$total.harv[modDat$treat=="mayAug"]),
                                                                    meanlog = median(pars.ma[,1]),
                                                                    sdlog = median(pars.ma[,2]))),
                 treat=c(rep("observed",length(modDat$total.harv[modDat$treat=="mayAug"])),rep("pred",length(modDat$total.harv[modDat$treat=="mayAug"]))))
set.seed(3)
wd25Comp=data.frame(harv.acre=c(modDat$total.harv[modDat$treat=="wd25"],rlnorm(n=length(modDat$total.harv[modDat$treat=="wd25"]),
                                                                    meanlog = median(pars.wd25[,1]),
                                                                    sdlog = median(pars.wd25[,2]))),
                 treat=c(rep("observed",length(modDat$total.harv[modDat$treat=="wd25"])),rep("pred",length(modDat$total.harv[modDat$treat=="wd25"]))))
set.seed(3)
wd50Comp=data.frame(harv.acre=c(modDat$total.harv[modDat$treat=="wd50"],rlnorm(n=length(modDat$total.harv[modDat$treat=="wd50"]),
                                                                    meanlog = median(pars.wd50[,1]),
                                                                    sdlog = median(pars.wd50[,2]))),
                 treat=c(rep("observed",length(modDat$total.harv[modDat$treat=="wd50"])),rep("pred",length(modDat$total.harv[modDat$treat=="wd50"]))))
set.seed(3)
we50Comp=data.frame(harv.acre=c(modDat$total.harv[modDat$treat=="we50"],rlnorm(n=length(modDat$total.harv[modDat$treat=="we50"]),
                                                                    meanlog = median(pars.we50[,1]),
                                                                    sdlog = median(pars.we50[,2]))),
                 treat=c(rep("observed",length(modDat$total.harv[modDat$treat=="we50"])),rep("pred",length(modDat$total.harv[modDat$treat=="we50"]))))

a.p=ggplot(aComp)+theme_classic()+
  geom_density(aes(x=log(harv.acre),fill=treat),alpha=0.2)+
  scale_fill_viridis_d()+labs(x="Log(Annual Harvest per Acre)", y="Density", title = "Actual", fill=element_blank())
nw.p=ggplot(nwComp)+theme_classic()+
  geom_density(aes(x=log(harv.acre),fill=treat),alpha=0.2)+
  scale_fill_viridis_d()+labs(x="Log(Annual Harvest per Acre)", y="Density", title = "No Winter Data \n(April - October)", fill=element_blank())
ma.p=ggplot(maComp)+theme_classic()+
  geom_density(aes(x=log(harv.acre),fill=treat),alpha=0.2)+
  scale_fill_viridis_d()+labs(x="Log(Annual Harvest per Acre)", y="Density", title = "May - August \nData Only", fill=element_blank())
wd25.p=ggplot(wd25Comp)+theme_classic()+
  geom_density(aes(x=log(harv.acre),fill=treat),alpha=0.2)+
  scale_fill_viridis_d()+labs(x="Log(Annual Harvest per Acre)", y="Density", title = "25% of Weekday \nCreel Visits/Month \nRemoved", fill=element_blank())
wd50.p=ggplot(wd25Comp)+theme_classic()+
  geom_density(aes(x=log(harv.acre),fill=treat),alpha=0.2)+
  scale_fill_viridis_d()+labs(x="Log(Annual Harvest per Acre)", y="Density", title = "50% of Weekday \nCreel Visits/Month \nRemoved", fill=element_blank())
we50.p=ggplot(we50Comp)+theme_classic()+
  geom_density(aes(x=log(harv.acre),fill=treat),alpha=0.2)+
  scale_fill_viridis_d()+labs(x="Log(Annual Harvest per Acre)", y="Density", title = "50% of Weekdend \nCreel Visits/Month \nRemoved", fill=element_blank())
...
gelmanDiagnostics(harv.actual) # 1.02 converged
gelmanDiagnostics(harv.nw) # 1.01 converged
gelmanDiagnostics(harv.ma) # 1.03 converged
gelmanDiagnostics(harv.25wd) # 1.02 converged
gelmanDiagnostics(harv.50wd) # 1.02 converged
gelmanDiagnostics(harv.50we) # 1.02 converged
...
actual.cv=sd(log(modDat$total.harv[modDat$treat=="actual"]))/mean(log(modDat$total.harv[modDat$treat=="actual"]))
...
noWinter.cv=sd(log(modDat$total.harv[modDat$treat=="noWinter"]))/mean(log(modDat$total.harv[modDat$treat=="noWinter"]))
...
mayAug.cv=sd(log(modDat$total.harv[modDat$treat=="mayAug"]))/mean(log(modDat$total.harv[modDat$treat=="mayAug"]))
...
wd25.cv=sd(log(modDat$total.harv[modDat$treat=="wd25"]))/mean(log(modDat$total.harv[modDat$treat=="wd25"]))
...
wd50.cv=sd(log(modDat$total.harv[modDat$treat=="wd50"]))/mean(log(modDat$total.harv[modDat$treat=="wd50"]))
...
we50.cv=sd(log(modDat$total.harv[modDat$treat=="we50"]))/mean(log(modDat$total.harv[modDat$treat=="we50"]))
...
Having established that the models are fitting the data well, some inference can be gained as to whether or not the reductions in creel effort proposed here would result in a meaningful change in the total annual harvest per acre estimated from the reduced data.

```{r harvDataComparisonToActual, fig.width=8,fig.height=6,fig.cap="Comparison of the distribution of actual annual harvest per acre estimates calculated from creel data and annual harvest per acre estimated calculated from simulated creel data for each data reduction scenario and the resulting median parameter values for their respective model fits."}

aComp=data.frame(harv.acre=c(modDat$total.harv[modDat$treat=="actual"],rlnorm(n=length(modDat$total.harv[modDat$treat=="actual"]),
                                                                    meanlog = median(pars.a[,1]),
                                                                    sdlog = median(pars.a[,2]))),
                 treat=c(rep("actual",length(modDat$total.harv[modDat$treat=="actual"])),rep("model",length(modDat$total.harv[modDat$treat=="actual"]))))

nwComp=data.frame(harv.acre=c(modDat$total.harv[modDat$treat=="actual"],rlnorm(n=length(modDat$total.harv[modDat$treat=="actual"]),
                                                                       meanlog = median(pars.nw[,1]),
                                                                       sdlog = median(pars.nw[,2]))),
                  treat=c(rep("actual",length(modDat$total.harv[modDat$treat=="actual"])),rep("model",length(modDat$total.harv[modDat$treat=="actual"]))))

maComp=data.frame(harv.acre=c(modDat$total.harv[modDat$treat=="actual"],rlnorm(n=length(modDat$total.harv[modDat$treat=="actual"]),
                                                                     meanlog = median(pars.ma[,1]),
                                                                     sdlog = median(pars.ma[,2]))),
                  treat=c(rep("actual",length(modDat$total.harv[modDat$treat=="actual"])),rep("model",length(modDat$total.harv[modDat$treat=="actual"]))))

wd25Comp=data.frame(harv.acre=c(modDat$total.harv[modDat$treat=="actual"],rlnorm(n=length(modDat$total.harv[modDat$treat=="actual"]),
                                                                     meanlog = median(pars.wd25[,1]),
                                                                     sdlog = median(pars.wd25[,2]))),
                    treat=c(rep("actual",length(modDat$total.harv[modDat$treat=="actual"])),rep("model",length(modDat$total.harv[modDat$treat=="actual"]))))

wd50Comp=data.frame(harv.acre=c(modDat$total.harv[modDat$treat=="actual"],rlnorm(n=length(modDat$total.harv[modDat$treat=="actual"]),
                                                                     meanlog = median(pars.wd50[,1]),
                                                                     sdlog = median(pars.wd50[,2]))),
                    treat=c(rep("actual",length(modDat$total.harv[modDat$treat=="actual"])),rep("model",length(modDat$total.harv[modDat$treat=="actual"]))))

we50Comp=data.frame(harv.acre=c(modDat$total.harv[modDat$treat=="actual"],rlnorm(n=length(modDat$total.harv[modDat$treat=="actual"]),
                                                                     meanlog = median(pars.we50[,1]),
                                                                     sdlog = median(pars.we50[,2]))),
                    treat=c(rep("actual",length(modDat$total.harv[modDat$treat=="actual"])),rep("model",length(modDat$total.harv[modDat$treat=="actual"]))))
...
The results of these tests suggest that when using either coefficient of variation or standard deviation as the variance metric to assess model fit and the $<0.10$ or $>0.90$ rule that only the may-august data reduction model is unable (Table \@ref(tab:bpValueComparison)).
...
kable(bpval.comp, digits = 3, col.names = c("Scenario", "CV p-value","SD p-value"), align="lcc",caption = "Bayesian p-values for the comparison between the actual data and the scenario-specific model fit to a lognormal distribution. This test is asking whether the scenario-specific model can approximate the actual data. When true this signals no effect of the data reduction for that scenario on the resulting annual harvest per acre estimate. Two metrics are assessed here, coefficient of variation (CV) and standard devidation (SD).")
```

### Year Specific Analyses

Instead of modeling the full population of annual harvest estimates as was done above, here I have done the model fitting on individual creel-years so that I can see how the removal of data for a creel-year (which is a comparatively large amount of missing data since only 16-20 lakes are creeled each year) impacts the annual harvest per acre estimated that year.
...
I first started with some basic analyses of the data on a year by year basis to understand how the data changes from year to year and if any lake characteristics explained variation (if there is any) in annual harvest per acre estimates across data reduction scenarios for specific creel-years (Figure \@ref(fig:yearAnalysisExampleLakes)).

```{r yearAnalysisExampleLakes, fig.width=8,fig.cap="Comparison of the annual harvest per acre estimated from the reduced data for a handful of creel surveys. Most data reductions do not result in annual harvest per acre estimates that are much different from the actual data (most are within 1 SD of the harvest estimate for that treatment). Colors represent data reduction scenarios, points are mean annual harvest per acre estimates across all months of that creel survey and vertical lines represent 1 standard deviation."}
ttrExp=ttrExp%>%
  left_join(cserv[,2:5])%>%
  left_join(lchar[,c(1:3,15)])
...
ggplot(pdat)+theme_classic()+
  geom_pointrange(aes(x=paste(year,waterbody.name,sep = "_"), 
                      y=total.harv, ymin=total.harv-sd(total.harv), 
                      ymax=total.harv+sd(total.harv), color=treat), 
                  position = position_dodge(width = 1))+
  theme(axis.text.x = element_text(angle=45,hjust=1), legend.position = c(.75,.75))+
  labs(x="Lake-Year",y="Annual Harvest per Acre (+/- 1 SD)",color="Scenario")+
  scale_color_viridis_d()

```

```{r year-analysis-diff-actual, fig.cap="Boxplot of the difference between the actual annual harvest per acre estimate and the data-reduced annual harvest per acre estimate for each creel survey in the creel dataset. Most of the data exhibits differences very near 0 except the seasonal reductions, dots represent outliers (x<|> x's percentile-1.5*interquartile range)"}
...
for(i in 1:nrow(trLake)){
  trLake$a.diff[i]=trLake$total.harv[trLake$treat=="actual" & trLake$survey.seq.no==trLake$survey.seq.no[i]]-trLake$total.harv[i]
  trLake$a.exceed[i]=trLake$a.diff[i]>=sd(trLake$total.harv[trLake$treat=="actual"])
}
...
kable(exceedSummary,
      ...
      caption = "Counts of the number of times the annual harvest per acre estimate for the reduced data scenario differs from the actual annual harvest per acre estimate for that treatment by more than 1 standard deviation. NAs ocurred where there was no variance in the harvest estimate and thus no standard deviation could be calculated for the actual data.")

```

```{r yearAnalysisRecCode, fig.cap="Comparison of the magnitutde of the difference between the mean actual annual harvest per acre estimate and the estimate derived from each reduced data set. Vertical lines represent 1 standard deviation. Walleye recruitment codes are along the x axis. Note the y-axis scale, many of the differences are small."}
...
ggplot(stClass)+theme_classic()+
  ...
  labs(x="WAE Recruitment Code",y="Mean Difference from Actual Annual Harvest Estimate",color="Scenario")


```
```{r yearAnalysisLakeClass,fig.cap="Comparison of the magnitude of the difference between the mean actual annual harvest per acre estimate and the estimate derived from each reduced data set. Vertical lines represent 1 standard deviation. Note the y-axis scale, many of the differences are very small."}
lcSum=trLake_chars%>%
...
ggplot(lcSum)+theme_classic()+
  ...
  labs(x="Lake Class",y="Mean Difference from Actual Annual Harvest Estimate", color="Scenario")
```
...
harvLL.we50=function(param){
  ...
  ll=dlnorm(tdat$total.harv[tdat$treat=="we50"], meanlog = mean(log(hvs)), sdlog = sd(log(hvs)), log = T)
  return(sum(ll))
}
harvLL.wd50=function(param){
  alpha=param[1]
  ...
  ll=dlnorm(tdat$total.harv[tdat$treat=="wd50"], meanlog = mean(log(hvs)), sdlog = sd(log(hvs)), log = T)
  return(sum(ll))
}
harvLL.wd25=function(param){
  alpha=param[1]
  ...
  ll=dlnorm(tdat$total.harv[tdat$treat=="wd25"], meanlog = mean(log(hvs)), sdlog = sd(log(hvs)), log = T)
  return(sum(ll))
}
harvLL.ma=function(param){
  alpha=param[1]
  ...
  ll=dlnorm(tdat$total.harv[tdat$treat=="mayAug"], meanlog = mean(log(hvs)), sdlog = sd(log(hvs)), log = T)
  return(sum(ll))
}
harvLL.nw=function(param){
  alpha=param[1]
  ...
  ll=dlnorm(tdat$total.harv[tdat$treat=="noWinter"], meanlog = mean(log(hvs)), sdlog = sd(log(hvs)), log = T)
  return(sum(ll))
}
harvLL.a=function(param){
  alpha=param[1]
  ...
  ll=dlnorm(tdat$total.harv[tdat$treat=="actual"], meanlog = mean(log(hvs)), sdlog = sd(log(hvs)), log = T)
  return(sum(ll))
}

# removing 0s
ttrExp=ttrExp[ttrExp$total.harv!=0,]
...
for(y in 1:length(loopY)){
  ...
  prior=createTruncatedNormalPrior(mean=c(mean(log(ttrExp$total.harv[ttrExp$treat=="actual"])),sd(log(ttrExp$total.harv[ttrExp$treat=="actual"]))),
                                   sd=c(1,1),
                                   lower = c(-15,0),
                                   upper = c(15,5))
  ...
  prior=createTruncatedNormalPrior(mean=c(mean(log(ttrExp$total.harv[ttrExp$treat=="noWinter"])),sd(log(ttrExp$total.harv[ttrExp$treat=="noWinter"]))),
                                   sd=c(1,1),
                                   lower = c(-15,0),
                                   upper = c(15,5))
  ...
  prior=createTruncatedNormalPrior(mean=c(mean(log(ttrExp$total.harv[ttrExp$treat=="mayAug"])),sd(log(ttrExp$total.harv[ttrExp$treat=="mayAug"]))),
                                   sd=c(1,1),
                                   lower = c(-15,0),
                                   upper = c(15,5))
  ...
  prior=createTruncatedNormalPrior(mean=c(mean(log(ttrExp$total.harv[ttrExp$treat=="wd25"])),sd(log(ttrExp$total.harv[ttrExp$treat=="wd25"]))),
                                   sd=c(1,1),
                                   lower = c(-15,0),
                                   upper = c(15,5))
  ...
  prior=createTruncatedNormalPrior(mean=c(mean(log(ttrExp$total.harv[ttrExp$treat=="wd50"])),sd(log(ttrExp$total.harv[ttrExp$treat=="wd50"]))),
                                   sd=c(1,1),
                                   lower = c(-15,0),
                                   upper = c(15,5))
  ...
  prior=createTruncatedNormalPrior(mean=c(mean(log(ttrExp$total.harv[ttrExp$treat=="we50"])),sd(log(ttrExp$total.harv[ttrExp$treat=="we50"]))),
                                   sd=c(1,1),
                                   lower = c(-15,0),
                                   upper = c(15,5))
  ...
  set.seed(10)
  for(i in 1:nrow(pars.a)){
    tempdat=rlnorm(n=length(tdat$total.harv[tdat$treat=="actual"]),
                   meanlog = pars.a[i,1],
                   sdlog = pars.a[i,2])
    pval.actual$alpha[i]=pars.a[i,1]
    pval.actual$beta[i]=pars.a[i,2]
  ...
  actual.cv=sd(log(tdat$total.harv[tdat$treat=="actual"]))/mean(log(tdat$total.harv[tdat$treat=="actual"]))
  actual.sd=sd(log(tdat$total.harv[tdat$treat=="actual"]))
  ...
  set.seed(10)
  for(i in 1:nrow(pars.nw)){
    tempdat=rlnorm(n=length(tdat$total.harv[tdat$treat=="noWinter"]),
                   meanlog = pars.nw[i,1],
                   sdlog = pars.nw[i,2])
    pval.noWinter$alpha[i]=pars.nw[i,1]
    pval.noWinter$beta[i]=pars.nw[i,2]
  ...
  noWinter.cv=sd(log(tdat$total.harv[tdat$treat=="noWinter"]))/mean(log(tdat$total.harv[tdat$treat=="noWinter"]))
  noWinter.sd=sd(log(tdat$total.harv[tdat$treat=="noWinter"]))
  ...
  set.seed(10)
  for(i in 1:nrow(pars.ma)){
    tempdat=rlnorm(n=length(tdat$total.harv[tdat$treat=="mayAug"]),
                   meanlog = pars.ma[i,1],
                   sdlog = pars.ma[i,2])
    pval.mayAug$alpha[i]=pars.ma[i,1]
    pval.mayAug$beta[i]=pars.ma[i,2]
  ...
  mayAug.cv=sd(log(tdat$total.harv[tdat$treat=="mayAug"]))/mean(log(tdat$total.harv[tdat$treat=="mayAug"]))
  mayAug.sd=sd(log(tdat$total.harv[tdat$treat=="mayAug"]))
  ...
  set.seed(10)
  for(i in 1:nrow(pars.wd25)){
    tempdat=rlnorm(n=length(tdat$total.harv[tdat$treat=="wd25"]),
                   meanlog = pars.wd25[i,1],
                   sdlog = pars.wd25[i,2])
    pval.wd25$alpha[i]=pars.wd25[i,1]
    pval.wd25$beta[i]=pars.wd25[i,2]
  ...
  wd25.cv=sd(log(tdat$total.harv[tdat$treat=="wd25"]))/mean(log(tdat$total.harv[tdat$treat=="wd25"]))
  wd25.sd=sd(log(tdat$total.harv[tdat$treat=="wd25"]))
  ...
  set.seed(10)
  for(i in 1:nrow(pars.wd50)){
    tempdat=rlnorm(n=length(tdat$total.harv[tdat$treat=="wd50"]),
                   meanlog = pars.wd50[i,1],
                   sdlog = pars.wd50[i,2])
    pval.wd50$alpha[i]=pars.wd50[i,1]
    pval.wd50$beta[i]=pars.wd50[i,2]
  ...
  wd50.cv=sd(log(tdat$total.harv[tdat$treat=="wd50"]))/mean(log(tdat$total.harv[tdat$treat=="wd50"]))
  wd50.sd=sd(log(tdat$total.harv[tdat$treat=="wd50"]))
  ...
  set.seed(10)
  for(i in 1:nrow(pars.we50)){
    tempdat=rlnorm(n=length(tdat$total.harv[tdat$treat=="we50"]),
                   meanlog = pars.we50[i,1],
                   sdlog = pars.we50[i,2])
    pval.we50$alpha[i]=pars.we50[i,1]
    pval.we50$beta[i]=pars.we50[i,2]
  ...
  we50.cv=sd(log(tdat$total.harv[tdat$treat=="we50"]))/mean(log(tdat$total.harv[tdat$treat=="we50"]))
  we50.sd=sd(log(tdat$total.harv[tdat$treat=="we50"]))
...
#saveRDS(yearLoopOutput, file = "yearLoopOutput_harvest_4.4.25.RData")
...
yearLoopOutput=readRDS("yearLoopOutput_harvest_4.4.25.RData")
...
ggplot(ttrExp)+theme_classic()+
  geom_density(aes(log(total.harv),fill=treat),alpha=0.2)+
  facet_wrap(~year,scales = 'free_y')+scale_fill_viridis_d()+
  theme(legend.position = "bottom")+
  labs(y="Density",x="Log(Annual Harvest per Acre)", fill="Scenario")
```
...
```
The results of these creel-year specific analyses suggest that even on an individual creel year level, the estimated annual harvest per acre from the data-reduced scenario generally is not significantly different from the annual harvest per acres calculated from the full data set (Figure \@ref(fig:yearLoopComparisonToActual)).
Compared to the same analysis for the full population of creel data all at once instead of on a year-by-year basis, the estimated harvest from the reduced data here match the actual quite as well for the seasonal data removals. All models do appear to  fit well and converge (Figure \@ref(fig:yearByyearConvergence)).

There did not appear to be any obvious characteristics of the lakes themselves or their walleye populations that correlated with the magnitude of the difference between the actual estimated annual harvest per acre and the reduced-data estimation.
Walleye recruitment code (Figure \@ref(fig:yearAnalysisRecCode)) and lake class (Figure \@ref(fig:yearAnalysisLakeClass)) were examined and showed no clear trend.
...
1.  Accuracy measure. In order to compare the harvest estimates from the reduced data to the actual data I used 1 standard deviation as my measure. This means that a reduced-data harvest estimate that is within 1 SD of the actual data harvest estimate was considered to be 'the same'. There may be better metrics to use here, and different people may have different opinions about the best metric to use. My rationale for using this metric has been outlined above.
...
A cursory look at the ability of gamma distributed models showed no real difference from lognormal and the definition of the beta better represents the data we're dealing with than the gamma definition so further analyses with this family of models was not pursued (Figure \@ref(fig:quickGammaFit)).
...
ttrExp=ttrExp[ttrExp$total.harv!=0,]
ggplot(ttrExp)+theme_classic()+
  geom_density(aes(x=total.harv, fill=treat),alpha=0.2)
...
harvLL.a.gamma=function(param){
  ...
  ll=dgamma(ttrExp$total.harv[ttrExp$treat=="actual"], shape = mu^2/sigma2, rate = mu/sigma2, log = T)
  return(sum(ll))
}
...
startT=Sys.time()
set.seed(10)
harv.actual.gamma=runMCMC(bayesianSetup = setup.actual, sampler = "DEzs", settings = settings)
...
harvLL.a=function(param){
  ...
  ll=dlnorm(ttrExp$total.harv[ttrExp$treat=="actual"], meanlog = mean(log(hrv)), sdlog = sd(log(hrv)), log = T)
  return(sum(ll))
}
...
#### NW ####
harvLL.nw.gamma=function(param){
  shape=param[1]
  rate=param[2]

  hrv=rgamma(nrow(ttrExp[ttrExp$treat=="noWinter",]), shape = shape, rate = rate)
  mu=mean(hrv,na.rm = T)
  sigma2=var(hrv,na.rm = T)
  
  ll=dgamma(ttrExp$total.harv[ttrExp$treat=="noWinter"], shape = mu^2/sigma2, rate = mu/sigma2, log = T)
  return(sum(ll))
}

prior.gamma=createUniformPrior(lower=c(0,0), upper = c(5,5))
setup.nw.gamma=createBayesianSetup(harvLL.nw.gamma, prior = prior.gamma)

settings=list(iterations=20000, nrChains=3, message=F, burnin=5000)

set.seed(10)
harv.nw.gamma=runMCMC(bayesianSetup = setup.nw.gamma, sampler = "DEzs", settings = settings)

#### MA ####

harvLL.ma.gamma=function(param){
  shape=param[1]
  rate=param[2]

  hrv=rgamma(nrow(ttrExp[ttrExp$treat=="mayAug",]), shape = shape, rate = rate)
  mu=mean(hrv,na.rm = T)
  sigma2=var(hrv,na.rm = T)
  
  ll=dgamma(ttrExp$total.harv[ttrExp$treat=="mayAug"], shape = mu^2/sigma2, rate = mu/sigma2, log = T)
  return(sum(ll))
}

prior.gamma=createUniformPrior(lower=c(0,0), upper = c(5,5))
setup.ma.gamma=createBayesianSetup(harvLL.ma.gamma, prior = prior.gamma)

settings=list(iterations=20000, nrChains=3, message=F, burnin=5000)

set.seed(10)
harv.ma.gamma=runMCMC(bayesianSetup = setup.ma.gamma, sampler = "DEzs", settings = settings)

#### WD25 ####

harvLL.wd25.gamma=function(param){
  shape=param[1]
  rate=param[2]

  hrv=rgamma(nrow(ttrExp[ttrExp$treat=="wd25",]), shape = shape, rate = rate)
  mu=mean(hrv,na.rm = T)
  sigma2=var(hrv,na.rm = T)
  
  ll=dgamma(ttrExp$total.harv[ttrExp$treat=="wd25"], shape = mu^2/sigma2, rate = mu/sigma2, log = T)
  return(sum(ll))
}

prior.gamma=createUniformPrior(lower=c(0,0), upper = c(5,5))
setup.wd25.gamma=createBayesianSetup(harvLL.wd25.gamma, prior = prior.gamma)

settings=list(iterations=20000, nrChains=3, message=F, burnin=5000)
set.seed(10)
harv.25wd.gamma=runMCMC(bayesianSetup = setup.wd25.gamma, sampler = "DEzs", settings = settings)

#### WD50 ####
harvLL.wd50.gamma=function(param){
  shape=param[1]
  rate=param[2]

  hrv=rgamma(nrow(ttrExp[ttrExp$treat=="wd50",]), shape = shape, rate = rate)
  mu=mean(hrv,na.rm = T)
  sigma2=var(hrv,na.rm = T)
  
  ll=dgamma(ttrExp$total.harv[ttrExp$treat=="wd50"], shape = mu^2/sigma2, rate = mu/sigma2, log = T)
  return(sum(ll))
}

prior.gamma=createUniformPrior(lower=c(0,0), upper = c(5,5))
setup.wd50.gamma=createBayesianSetup(harvLL.wd50.gamma, prior = prior.gamma)

settings=list(iterations=20000, nrChains=3, message=F, burnin=5000)

set.seed(10)
harv.50wd.gamma=runMCMC(bayesianSetup = setup.wd50.gamma, sampler = "DEzs", settings = settings)

#### WE50 ####
harvLL.we50.gamma=function(param){
  shape=param[1]
  rate=param[2]

  hrv=rgamma(nrow(ttrExp[ttrExp$treat=="we50",]), shape = shape, rate = rate)
  mu=mean(hrv,na.rm = T)
  sigma2=var(hrv,na.rm = T)
  
  ll=dgamma(ttrExp$total.harv[ttrExp$treat=="we50"], shape = mu^2/sigma2, rate = mu/sigma2, log = T)
  return(sum(ll))
}

prior.gamma=createUniformPrior(lower=c(0,0), upper = c(5,5))
setup.we50.gamma=createBayesianSetup(harvLL.we50.gamma, prior = prior.gamma)

settings=list(iterations=20000, nrChains=3, message=F, burnin=5000)

set.seed(10)
harv.50we.gamma=runMCMC(bayesianSetup = setup.we50.gamma, sampler = "DEzs", settings = settings)
```
...
pars.a.lnorm=getSample(harv.actual)
pars.a.gamma=getSample(harv.actual.gamma)

set.seed(10)
aComp.lnorm=data.frame(harv.acre=c(ttrExp$total.harv[ttrExp$treat=="actual"],rlnorm(n=length(ttrExp$total.harv[ttrExp$treat=="actual"]),
                                                                    meanlog = median(pars.a.lnorm[,1]),
                                                                    sdlog = median(pars.a.lnorm[,2]))),
                 treat=c(rep("observed",length(ttrExp$total.harv[ttrExp$treat=="actual"])),rep("pred",length(ttrExp$total.harv[ttrExp$treat=="actual"]))))

aComp.gamma=data.frame(harv.acre=c(ttrExp$total.harv[ttrExp$treat=="actual"],rgamma(n=length(ttrExp$total.harv[ttrExp$treat=="actual"]),
                                                                    shape = median(pars.a.gamma[,1]),
                                                                    rate = median(pars.a.gamma[,2]))),
                  treat=c(rep("observed",length(ttrExp$total.harv[ttrExp$treat=="actual"])),rep("pred",length(ttrExp$total.harv[ttrExp$treat=="actual"]))))

ln=ggplot(aComp.lnorm)+theme_classic()+
  geom_density(aes(x=harv.acre, fill=treat),alpha=0.3)+
  scale_fill_viridis_d()+labs(x="Annual Harvest per Acre",title = "Lognormal")+
  theme(legend.position = c(0.75,0.75))
gma=ggplot(aComp.gamma)+theme_classic()+
  geom_density(aes(x=harv.acre, fill=treat),alpha=0.3)+
  scale_fill_viridis_d()+labs(x="Annual Harvest per Acre",title = "Gamma")+
  theme(legend.position = c(0.75,0.75))
ggarrange(ln,gma)

## It looks like the gamma models are fitting the data well, and could be a good alternative choice, so stick with lognormal for now though
```
...
# looking to see if parm estimates produce data that visually at least looks like the observed data for that scenario
pars.a.gamma=getSample(harv.actual.gamma)
pars.nw.gamma=getSample(harv.nw.gamma)
pars.ma.gamma=getSample(harv.ma.gamma)
pars.wd25.gamma=getSample(harv.25wd.gamma)
pars.wd50.gamma=getSample(harv.50wd.gamma)
pars.we50.gamma=getSample(harv.50we.gamma)
set.seed(10)
aComp.gamma=data.frame(harv.acre=c(ttrExp$total.harv[ttrExp$treat=="actual"],rgamma(n=length(ttrExp$total.harv[ttrExp$treat=="actual"]),
                                                                    shape = median(pars.a.gamma[,1]),
                                                                    rate = median(pars.a.gamma[,2]))),
                  treat=c(rep("observed",length(ttrExp$total.harv[ttrExp$treat=="actual"])),rep("pred",length(ttrExp$total.harv[ttrExp$treat=="actual"]))))
nwComp.gamma=data.frame(harv.acre=c(ttrExp$total.harv[ttrExp$treat=="noWinter"],rgamma(n=length(ttrExp$total.harv[ttrExp$treat=="noWinter"]),
                                                                      shape = median(pars.nw.gamma[,1]),
                                                                      rate = median(pars.nw.gamma[,2]))),
                  treat=c(rep("observed",length(ttrExp$total.harv[ttrExp$treat=="noWinter"])),rep("pred",length(ttrExp$total.harv[ttrExp$treat=="noWinter"]))))

maComp.gamma=data.frame(harv.acre=c(ttrExp$total.harv[ttrExp$treat=="mayAug"],rgamma(n=length(ttrExp$total.harv[ttrExp$treat=="mayAug"]),
                                                                    shape = median(pars.ma.gamma[,1]),
                                                                    rate = median(pars.ma.gamma[,2]))),
                  treat=c(rep("observed",length(ttrExp$total.harv[ttrExp$treat=="mayAug"])),rep("pred",length(ttrExp$total.harv[ttrExp$treat=="mayAug"]))))

wd25Comp.gamma=data.frame(harv.acre=c(ttrExp$total.harv[ttrExp$treat=="wd25"],rgamma(n=length(ttrExp$total.harv[ttrExp$treat=="wd25"]),
...
wd50Comp.gamma=data.frame(harv.acre=c(ttrExp$total.harv[ttrExp$treat=="wd50"],rgamma(n=length(ttrExp$total.harv[ttrExp$treat=="wd50"]),
...
we50Comp.gamma=data.frame(harv.acre=c(ttrExp$total.harv[ttrExp$treat=="we50"],rgamma(n=length(ttrExp$total.harv[ttrExp$treat=="we50"]),
...
a.p.gamma=ggplot(aComp.gamma)+theme_classic()+
  geom_density(aes(x=harv.acre,fill=treat),alpha=0.2)+
...
actual.cv=sd(ttrExp$total.harv[ttrExp$treat=="actual"])/mean(ttrExp$total.harv[ttrExp$treat=="actual"])
...
noWinter.cv=sd(ttrExp$total.harv[ttrExp$treat=="noWinter"])/mean(ttrExp$total.harv[ttrExp$treat=="noWinter"])
...
mayAug.cv=sd(ttrExp$total.harv[ttrExp$treat=="mayAug"])/mean(ttrExp$total.harv[ttrExp$treat=="mayAug"])
...
wd25.cv=sd(ttrExp$total.harv[ttrExp$treat=="wd25"])/mean(ttrExp$total.harv[ttrExp$treat=="wd25"])
...
wd50.cv=sd(ttrExp$total.harv[ttrExp$treat=="wd50"])/mean(ttrExp$total.harv[ttrExp$treat=="wd50"])
...
we50.cv=sd(ttrExp$total.harv[ttrExp$treat=="we50"])/mean(ttrExp$total.harv[ttrExp$treat=="we50"])
...
This means that the gamma distribution not likely to be appropriate for making comparisons between the actual data and the reduced data to understand whether or not creel effort reductions result in significantly different annual harvest per acre estimates or not.
