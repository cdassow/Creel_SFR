...
> **How much less creel data could be collected, on an individual survey basis while still maintaining the stratified random sampling design, and still produce harvest rate estimates that are NOT significantly different from harvest rate estimates obtained using the current creel survey design?**
...
Here I'm using harvest rate estimates for ceded territory lakes only.
I'm not working with a mean harvest per month or day type or anything like that because I think the metric of importance here is total harvest rate.


What I've done here is calculate harvest rate based on the full data sets as well as for 5 scenarios:
...
iharvR=charvR%>% # making a separate data frame to add groupings to so I can leave charv alone
  filter(species.code=="X22" & wbic%in%ctwiWBIC.creel)%>%
  mutate(season=ifelse(month%in%4:10,"openwater","winter"))
...
harvestR.nw=iharvR%>%
  filter(season=="openwater")%>%
  select(year,wbic,waterbody.name,survey.seq.no, month, daytype,spp.harvest.rate,harvest.rate)
...
A good model fit here is evident when the distribution of the observed and predicted data overlap nearly entirely (Figure \@ref(fig:harvDataReproduction)).
...
Lastly, a Bayesian p-value was calculated for each model to further describe model fit to the data (Figure \@ref(tab:harvbpValue)).
...
Thus, with Bayesian p-values a value of 0.5 is ideal as it means the model can generate data that matches the distribution of the observed data really well (Table \@ref(tab:harvbpValue), \@ref(tab:bpValueComparison).
...
gelmanDiagnostics(harv.actual) # 1.03 converged 
gelmanDiagnostics(harv.nw) # 1.02 converged
gelmanDiagnostics(harv.ma) # 1.02 converged
gelmanDiagnostics(harv.25wd) # 1.03 converged
gelmanDiagnostics(harv.50wd) # 1.01 converged
gelmanDiagnostics(harv.50we) # 1.03 converged
...
actual.cv=sd(log(modDat$meanHarvR[modDat$treat=="actual"]))/mean(log(modDat$meanHarvR[modDat$treat=="actual"]))
...
noWinter.cv=sd(log(modDat$meanHarvR[modDat$treat=="noWinter"]))/mean(log(modDat$meanHarvR[modDat$treat=="noWinter"]))
...
mayAug.cv=sd(log(modDat$meanHarvR[modDat$treat=="mayAug"]))/mean(log(modDat$meanHarvR[modDat$treat=="mayAug"]))
...
wd25.cv=sd(log(modDat$meanHarvR[modDat$treat=="wd25"]))/mean(log(modDat$meanHarvR[modDat$treat=="wd25"]))
...
wd50.cv=sd(log(modDat$meanHarvR[modDat$treat=="wd50"]))/mean(log(modDat$meanHarvR[modDat$treat=="wd50"]))
...
we50.cv=sd(log(modDat$meanHarvR[modDat$treat=="we50"]))/mean(log(modDat$meanHarvR[modDat$treat=="we50"]))
...
The results of these tests suggest that when using either coefficient of variation or standard deviation as the variance metric to assess model fit and the $<0.10$ or $>0.90$ rule all scenarios can adequately reproduce the actual data (Table \@ref(tab:bpValueComparison)).
...
```
```{r yearAnalysisLakeClass,fig.cap="Comparison of the magnitude of the difference between the mean actual harvest rate and the estimate derived from each reduced data set. Vertical lines represent 1 standard deviation. Note the y-axis scale, many of the differences are very small."}
lcSum=trLake_chars%>%
...
for(y in 1:length(loopY)){
  ...
  actual.cv=sd(log(tdat$meanHarvR[tdat$treat=="actual"]))/mean(log(tdat$meanHarvR[tdat$treat=="actual"]))
  ...
  noWinter.cv=sd(log(tdat$meanHarvR[tdat$treat=="noWinter"]))/mean(log(tdat$meanHarvR[tdat$treat=="noWinter"]))
  ...
  mayAug.cv=sd(log(tdat$meanHarvR[tdat$treat=="mayAug"]))/mean(log(tdat$meanHarvR[tdat$treat=="mayAug"]))
  ...
  wd25.cv=sd(log(tdat$meanHarvR[tdat$treat=="wd25"]))/mean(log(tdat$meanHarvR[tdat$treat=="wd25"]))
  ...
  wd50.cv=sd(log(tdat$meanHarvR[tdat$treat=="wd50"]))/mean(log(tdat$meanHarvR[tdat$treat=="wd50"]))
  ...
  we50.cv=sd(log(tdat$meanHarvR[tdat$treat=="we50"]))/mean(log(tdat$meanHarvR[tdat$treat=="we50"]))
...
#saveRDS(yearLoopOutput, file = "yearLoopOutput_harvestR_4.4.25.RData")
...
yearLoopOutput=readRDS("yearLoopOutput_harvestR_4.4.25.RData")
...
```
The results of these creel-year specific analyses suggest that even on an individual creel year level, the harvest rate from the data-reduced scenario generally is not significantly different from the harvest rate calculated from the full data set (Figure \@ref(fig:yearLoopComparisonToActual)).
All models do appear to  fit well and converge (Figure \@ref(fig:yearByyearConvergence)).

There did not appear to be any obvious characteristics of the lakes themselves or their walleye populations that correlated with the magnitude of the difference between the actual harvest rate and the reduced-data estimation.
...
1.  Accuracy measure. In order to compare the harvest rates from the reduced data to the actual data I used 1 standard deviation as my measure. This means that a reduced-data harvest rate that is within 1 SD of the actual data harvest rate was considered to be 'the same'. There may be better metrics to use here, and different people may have different opinions about the best metric to use. My rationale for using this metric has been outlined above.
...
## It looks like the gamma and beta models are fitting the data well, and could be a good alternative choices, so stick with lognormal for now though
```
...
Seems like the beta models could be a potential alternative, the bayesian p values should help confirm this.

```{r betaGRDiagnostics, eval=F, echo=T}
### MODEL CHECKNG ####

gelmanDiagnostics(harvR.actual.beta) # 1.02 converged
gelmanDiagnostics(harvR.nw.beta) # 1.02 converged
gelmanDiagnostics(harvR.ma.beta) # 1.02 converged
gelmanDiagnostics(harvR.25wd.beta) # 1.03 converged
gelmanDiagnostics(harvR.50wd.beta) # 1.03 converged
gelmanDiagnostics(harvR.50we.beta) # 1.02 converged
...
actual.cv=sd(ttrExp$meanHarvR[ttrExp$treat=="actual"])/mean(ttrExp$meanHarvR[ttrExp$treat=="actual"])
actual.sd=sd(ttrExp$meanHarvR[ttrExp$treat=="actual"])

pval.actual$cvExceed[pval.actual$cv>actual.cv]=1
pval.actual$sdExceed[pval.actual$sd>actual.sd]=1

bpval.self.beta$coef.var.pval[1]=sum(pval.actual$cvExceed==1)/nrow(pval.actual) # if this value is < 0.1 for >0.9 that would suggest model is not fitting well. 0.5 is ideal
...
noWinter.cv=sd(ttrExp$meanHarvR[ttrExp$treat=="noWinter"])/mean(ttrExp$meanHarvR[ttrExp$treat=="noWinter"])
noWinter.sd=sd(ttrExp$meanHarvR[ttrExp$treat=="noWinter"])

pval.noWinter$cvExceed[pval.noWinter$cv>noWinter.cv]=1
pval.noWinter$sdExceed[pval.noWinter$sd>noWinter.sd]=1

bpval.self.beta$coef.var.pval[2]=sum(pval.noWinter$cvExceed==1)/nrow(pval.noWinter) # if this value is < 0.1 for >0.9 that would suggest model is not fitting well. 0.5 is ideal
...
mayAug.cv=sd(ttrExp$meanHarvR[ttrExp$treat=="mayAug"])/mean(ttrExp$meanHarvR[ttrExp$treat=="mayAug"])
mayAug.sd=sd(ttrExp$meanHarvR[ttrExp$treat=="mayAug"])

pval.mayAug$cvExceed[pval.mayAug$cv>mayAug.cv]=1
pval.mayAug$sdExceed[pval.mayAug$sd>mayAug.sd]=1

bpval.self.beta$coef.var.pval[3]=sum(pval.mayAug$cvExceed==1)/nrow(pval.mayAug) # if this value is < 0.1 for >0.9 that would suggest model is not fitting well. 0.5 is ideal
...
wd25.cv=sd(ttrExp$meanHarvR[ttrExp$treat=="wd25"])/mean(ttrExp$meanHarvR[ttrExp$treat=="wd25"])
wd25.sd=sd(ttrExp$meanHarvR[ttrExp$treat=="wd25"])

pval.wd25$cvExceed[pval.wd25$cv>wd25.cv]=1
pval.wd25$sdExceed[pval.wd25$sd>wd25.sd]=1

bpval.self.beta$coef.var.pval[4]=sum(pval.wd25$cvExceed==1)/nrow(pval.wd25) # if this value is < 0.1 for >0.9 that would suggest model is not fitting well. 0.5 is ideal
...
wd50.cv=sd(ttrExp$meanHarvR[ttrExp$treat=="wd50"])/mean(ttrExp$meanHarvR[ttrExp$treat=="wd50"])
wd50.sd=sd(ttrExp$meanHarvR[ttrExp$treat=="wd50"])

pval.wd50$cvExceed[pval.wd50$cv>wd50.cv]=1
pval.wd50$sdExceed[pval.wd50$sd>wd50.sd]=1

bpval.self.beta$coef.var.pval[5]=sum(pval.wd50$cvExceed==1)/nrow(pval.wd50) # if this value is < 0.1 for >0.9 that would suggest model is not fitting well. 0.5 is ideal
...
we50.cv=sd(ttrExp$meanHarvR[ttrExp$treat=="we50"])/mean(ttrExp$meanHarvR[ttrExp$treat=="we50"])
we50.sd=sd(ttrExp$meanHarvR[ttrExp$treat=="we50"])

pval.we50$cvExceed[pval.we50$cv>we50.cv]=1
pval.we50$sdExceed[pval.we50$sd>we50.sd]=1

bpval.self.beta$coef.var.pval[6]=sum(pval.we50$cvExceed==1)/nrow(pval.we50) # if this value is < 0.1 for >0.9 that would suggest model is not fitting well. 0.5 is ideal
...
kable(bpval.self.beta, digits = 3, col.names = c("Scenario", "CV p-value","SD p-value"), align="lcc",caption = "Bayesian p-values for a beta distributed model. Each p-value describes whether or not there is a significant difference between data generated by the fitted model and the actual data for that scenario. Two metrics are assessed here, coefficient of variation (CV) and standard devidation (SD).")
...
#                      sum(pval.we50$sdComp)/nrow(pval.we50))
# 
# kable(bpval.comp, digits = 3, col.names = c("Scenario", "CV p-value","SD p-value"), align="lcc",caption = "Bayesian p-values for a shape2 distributed model. Each p-value describes whether or not there is a significant difference between data generated by the fitted model and the actual data for that scenario. Two metrics are assessed here, coefficient of variation (CV) and standard devidation (SD).")
```
