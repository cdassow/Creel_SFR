...
ceff.ct=ceff%>%
  ...
  mutate(annual.eff.hectare=total.effort/(lake.area/2.47)) # conversion to hectares
...
p1=ggplot(ceff.ct)+theme_classic()+
  geom_boxplot(aes(y=annual.eff.hectare, x=plot.month), fill="grey")+
  geom_hline(yintercept = mean(ceff.ct$annual.eff.hectare, na.rm = T), color="red")+
  labs(y=" Annual Effort per Hectare", x="Month")

# plot of effort by month - logged
p2=ggplot(ceff.ct)+theme_classic()+
  geom_boxplot(aes(y=log(annual.eff.hectare), x=plot.month), fill="grey")+
  geom_hline(yintercept = log(mean(ceff.ct$annual.eff.hectare)), color="red")+ # setting this manually since the 0 exp rates mess up the logging here
  labs(y="Log(Annual Effort per Hectare)", x="Month")
...
eff.surv.a=ceff.ct%>%
  group_by(survey.seq.no)%>%
  summarise(total.eff=sum(annual.eff.hectare,na.rm = T))
eff.surv.nw=eff.nw%>%
  group_by(survey.seq.no)%>%
  summarise(total.eff=sum(annual.eff.hectare,na.rm = T))
eff.surv.ma=eff.ma%>%
...
set.seed(3)
aComp=data.frame(eff.hectare=c(modDat$total.eff[modDat$treat=="actual"],rlnorm(n=length(modDat$total.eff[modDat$treat=="actual"]),
...
set.seed(3)
nwComp=data.frame(eff.hectare=c(modDat$total.eff[modDat$treat=="noWinter"],rlnorm(n=length(modDat$total.eff[modDat$treat=="noWinter"]),
...
set.seed(3)
maComp=data.frame(eff.hectare=c(modDat$total.eff[modDat$treat=="mayAug"],rlnorm(n=length(modDat$total.eff[modDat$treat=="mayAug"]),
...
set.seed(3)
wd25Comp=data.frame(eff.hectare=c(modDat$total.eff[modDat$treat=="wd25"],rlnorm(n=length(modDat$total.eff[modDat$treat=="wd25"]),
...
set.seed(3)
wd50Comp=data.frame(eff.hectare=c(modDat$total.eff[modDat$treat=="wd50"],rlnorm(n=length(modDat$total.eff[modDat$treat=="wd50"]),
...
set.seed(3)
we50Comp=data.frame(eff.hectare=c(modDat$total.eff[modDat$treat=="we50"],rlnorm(n=length(modDat$total.eff[modDat$treat=="we50"]),
...
a.p=ggplot(aComp)+theme_classic()+
  geom_density(aes(x=log(eff.hectare),fill=treat),alpha=0.2)+
...
kable(bpValues, digits = 3, col.names = c("Scenario","SD p-value"), align="lcc",caption = "Bayesian p-values for a lognormal distributed model. Each p-value describes whether or not there is a significant difference between data generated by the fitted model and the actual data for that scenario. Two metrics is standard devidation (SD).")
```
...
nwComp=data.frame(eff.hectare=c(modDat$total.eff[modDat$treat=="actual"],rlnorm(n=length(modDat$total.eff[modDat$treat=="actual"]),
                                                                       meanlog = median(pars.nw[,1]),
                                                                       sdlog = median(pars.nw[,2]))),
                  treat=c(rep("actual",length(modDat$total.eff[modDat$treat=="actual"])),rep("model",length(modDat$total.eff[modDat$treat=="actual"]))))

maComp=data.frame(eff.hectare=c(modDat$total.eff[modDat$treat=="actual"],rlnorm(n=length(modDat$total.eff[modDat$treat=="actual"]),
                                                                     meanlog = median(pars.ma[,1]),
                                                                     sdlog = median(pars.ma[,2]))),
                  treat=c(rep("actual",length(modDat$total.eff[modDat$treat=="actual"])),rep("model",length(modDat$total.eff[modDat$treat=="actual"]))))

wd25Comp=data.frame(eff.hectare=c(modDat$total.eff[modDat$treat=="actual"],rlnorm(n=length(modDat$total.eff[modDat$treat=="actual"]),
                                                                     meanlog = median(pars.wd25[,1]),
                                                                     sdlog = median(pars.wd25[,2]))),
                    treat=c(rep("actual",length(modDat$total.eff[modDat$treat=="actual"])),rep("model",length(modDat$total.eff[modDat$treat=="actual"]))))

wd50Comp=data.frame(eff.hectare=c(modDat$total.eff[modDat$treat=="actual"],rlnorm(n=length(modDat$total.eff[modDat$treat=="actual"]),
                                                                     meanlog = median(pars.wd50[,1]),
                                                                     sdlog = median(pars.wd50[,2]))),
                    treat=c(rep("actual",length(modDat$total.eff[modDat$treat=="actual"])),rep("model",length(modDat$total.eff[modDat$treat=="actual"]))))

we50Comp=data.frame(eff.hectare=c(modDat$total.eff[modDat$treat=="actual"],rlnorm(n=length(modDat$total.eff[modDat$treat=="actual"]),
                                                                     meanlog = median(pars.we50[,1]),
                                                                     sdlog = median(pars.we50[,2]))),
                    treat=c(rep("actual",length(modDat$total.eff[modDat$treat=="actual"])),rep("model",length(modDat$total.eff[modDat$treat=="actual"]))))
...
#saveRDS(yearLoopOutput, file = "yearLoopOutput_effort_9.23.25.RData")
...
yearLoopOutput=readRDS("yearLoopOutput_effort_9.23.25.RData")
...
set.seed(10)
aComp.lnorm=data.frame(eff.hectare=c(ttrExp$total.eff[ttrExp$treat=="actual"],rlnorm(n=length(ttrExp$total.eff[ttrExp$treat=="actual"]),
...
aComp.gamma=data.frame(eff.hectare=c(ttrExp$total.eff[ttrExp$treat=="actual"],rgamma(n=length(ttrExp$total.eff[ttrExp$treat=="actual"]),
                                                                    shape = median(pars.a.gamma[,1]),
                                                                    rate = median(pars.a.gamma[,2]))),
                  treat=c(rep("observed",length(ttrExp$total.eff[ttrExp$treat=="actual"])),rep("pred",length(ttrExp$total.eff[ttrExp$treat=="actual"]))))

ln=ggplot(aComp.lnorm)+theme_classic()+
  geom_density(aes(x=eff.hectare, fill=treat),alpha=0.3)+
...
aComp.gamma=data.frame(eff.hectare=c(ttrExp$total.eff[ttrExp$treat=="actual"],rgamma(n=length(ttrExp$total.eff[ttrExp$treat=="actual"]),
                                                                    ...
                                                                    rate = median(pars.a.gamma[,2]))),
                  treat=c(rep("observed",length(ttrExp$total.eff[ttrExp$treat=="actual"])),rep("pred",length(ttrExp$total.eff[ttrExp$treat=="actual"]))))
nwComp.gamma=data.frame(eff.hectare=c(ttrExp$total.eff[ttrExp$treat=="noWinter"],rgamma(n=length(ttrExp$total.eff[ttrExp$treat=="noWinter"]),
...
maComp.gamma=data.frame(eff.hectare=c(ttrExp$total.eff[ttrExp$treat=="mayAug"],rgamma(n=length(ttrExp$total.eff[ttrExp$treat=="mayAug"]),
...
wd25Comp.gamma=data.frame(eff.hectare=c(ttrExp$total.eff[ttrExp$treat=="wd25"],rgamma(n=length(ttrExp$total.eff[ttrExp$treat=="wd25"]),
...
wd50Comp.gamma=data.frame(eff.hectare=c(ttrExp$total.eff[ttrExp$treat=="wd50"],rgamma(n=length(ttrExp$total.eff[ttrExp$treat=="wd50"]),
...
we50Comp.gamma=data.frame(eff.hectare=c(ttrExp$total.eff[ttrExp$treat=="we50"],rgamma(n=length(ttrExp$total.eff[ttrExp$treat=="we50"]),
...
a.p.gamma=ggplot(aComp.gamma)+theme_classic()+
  geom_density(aes(x=eff.hectare,fill=treat),alpha=0.2)+coord_cartesian(xlim = c(0,250))+
...
kable(bpval.self.gamma, digits = 3, col.names = c("Scenario", "SD p-value"), align="lcc",caption = "Bayesian p-values for a gamma distributed model. Each p-value describes whether or not there is a significant difference between data generated by the fitted model and the actual data for that scenario. Standard devidation (SD) was the metric assessed here.")
