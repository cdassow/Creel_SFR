...
DNR has amassed a large amount of inland creel information over many decades (`r range(1984-2023)`) across many waterbodies (n=`r length(unique(cserv$wbic))`).
...
```
```{r yearAnalysisLakeClass,fig.cap="Comparison of the magnitude of the difference between the mean actual annual effort per acre estimate and the estimate derived from each reduced data set. Vertical lines represent 1 standard deviation. Note the y-axis scale, many of the differences are very small."}
lcSum=trLake_chars%>%
...
saveRDS(yearLoopOutput, file = "yearLoopOutput_effort_3.13.25.RData")
...
yearLoopOutput=readRDS("yearLoopOutput_effort_3.13.25.RData")
