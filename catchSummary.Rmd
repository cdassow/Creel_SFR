...
> **How much less creel data could be collected, on an individual survey basis while still maintaining the stratified random sampling design, and still produce catch estimates that are NOT significantly different from catch estimates obtained using the current creel survey design?**
...
Here I'm using catch estimates for ceded territory lakes only.
I'm not working with a mean catch per month or day type or anything like that because I think the metric of importance here is total catch.
I am using catch per acre to standardize for lake size, similar to what I did for effort because of the nature of the data. 
This isn't necessary for exploitation rate calcs for walleye because it's expressed as a rate already.

What I've done here is calculate catch based on the full data sets as well as for 5 scenarios:
...
```{r monthlyCatch,fig.width=8,fig.cap="Distribution of catch estimates by month (panel a) for all years of CTWI creel data. Horizontal red line is overall mean catch. Black points are outliers. Panel B is the same information presented on a log-scale. "}


# plot of catch estimate by month
ccatch.ct$plot.month=month(ccatch.ct$month,label = T)
p1=ggplot(ccatch.ct)+theme_classic()+
  geom_boxplot(aes(y=ccatch.ct$annual.catch.acre, x=plot.month), fill="grey")+
  geom_hline(yintercept = mean(ccatch.ct$annual.catch.acre, na.rm = T), color="red")+
  labs(y=" Annual Catch per Acre", x="Month")

# plot of catch estimate by month - logged
p2=ggplot(ccatch.ct)+theme_classic()+
  geom_boxplot(aes(y=log(ccatch.ct$annual.catch.acre), x=plot.month), fill="grey")+
  geom_hline(yintercept = log(mean(ccatch.ct$annual.catch.acre)), color="red")+ # setting this manually since the 0 exp rates mess up the logging here
  labs(y="Log(Annual Catch per Acre)", x="Month")
...
# creating survey-level catch estimates for the actual data, estimate of total catch for that creel year produced from the observed catch data
  ...
  group_by(survey.seq.no)%>%
  summarise(total.catch=sum(annual.catch.acre,na.rm = T))
...
ggplot(ttrExp)+theme_classic()+
  ...
  labs(x="Log(Annual Catch per Acre)", y="Density", fill="Scenario")
...
In order to estimate parameters for a model of the catch distribution that produced what is represented in the creel data a Bayesian modeling approach was used.
This is because the catch data are not normally distributed and thus don't meet the assumptions of typical analyses based on normality.
...
set.seed(3)
aComp=data.frame(catch.acre=c(modDat$total.catch[modDat$treat=="actual"],rlnorm(n=length(modDat$total.catch[modDat$treat=="actual"]),
...
set.seed(3)
nwComp=data.frame(catch.acre=c(modDat$total.catch[modDat$treat=="noWinter"],rlnorm(n=length(modDat$total.catch[modDat$treat=="noWinter"]),
...
set.seed(3)
maComp=data.frame(catch.acre=c(modDat$total.catch[modDat$treat=="mayAug"],rlnorm(n=length(modDat$total.catch[modDat$treat=="mayAug"]),
...
set.seed(3)
wd25Comp=data.frame(catch.acre=c(modDat$total.catch[modDat$treat=="wd25"],rlnorm(n=length(modDat$total.catch[modDat$treat=="wd25"]),
...
set.seed(3)
wd50Comp=data.frame(catch.acre=c(modDat$total.catch[modDat$treat=="wd50"],rlnorm(n=length(modDat$total.catch[modDat$treat=="wd50"]),
...
set.seed(3)
we50Comp=data.frame(catch.acre=c(modDat$total.catch[modDat$treat=="we50"],rlnorm(n=length(modDat$total.catch[modDat$treat=="we50"]),
...
a.p=ggplot(aComp)+theme_classic()+
  geom_density(aes(x=log(catch.acre),fill=treat),alpha=0.2)+
  scale_fill_viridis_d()+labs(x="Log(Annual Catch per Acre)", y="Density", title = "Actual", fill=element_blank())
nw.p=ggplot(nwComp)+theme_classic()+
  geom_density(aes(x=log(catch.acre),fill=treat),alpha=0.2)+
  scale_fill_viridis_d()+labs(x="Log(Annual Catch per Acre)", y="Density", title = "No Winter Data \n(April - October)", fill=element_blank())
ma.p=ggplot(maComp)+theme_classic()+
  geom_density(aes(x=log(catch.acre),fill=treat),alpha=0.2)+
  scale_fill_viridis_d()+labs(x="Log(Annual Catch per Acre)", y="Density", title = "May - August \nData Only", fill=element_blank())
wd25.p=ggplot(wd25Comp)+theme_classic()+
  geom_density(aes(x=log(catch.acre),fill=treat),alpha=0.2)+
  scale_fill_viridis_d()+labs(x="Log(Annual Catch per Acre)", y="Density", title = "25% of Weekday \nCreel Visits/Month \nRemoved", fill=element_blank())
wd50.p=ggplot(wd25Comp)+theme_classic()+
  geom_density(aes(x=log(catch.acre),fill=treat),alpha=0.2)+
  scale_fill_viridis_d()+labs(x="Log(Annual Catch per Acre)", y="Density", title = "50% of Weekday \nCreel Visits/Month \nRemoved", fill=element_blank())
we50.p=ggplot(we50Comp)+theme_classic()+
  geom_density(aes(x=log(catch.acre),fill=treat),alpha=0.2)+
  scale_fill_viridis_d()+labs(x="Log(Annual Catch per Acre)", y="Density", title = "50% of Weekdend \nCreel Visits/Month \nRemoved", fill=element_blank())
...
gelmanDiagnostics(catch.actual) # 1.01 converged
gelmanDiagnostics(catch.nw) # 1.02 converged
gelmanDiagnostics(catch.ma) # 1.02 converged
gelmanDiagnostics(catch.25wd) # 1.01 converged
gelmanDiagnostics(catch.50wd) # 1.02 converged
gelmanDiagnostics(catch.50we) # 1.02 converged
...
actual.cv=sd(log(modDat$total.catch[modDat$treat=="actual"]))/mean(log(modDat$total.catch[modDat$treat=="actual"]))
...
noWinter.cv=sd(log(modDat$total.catch[modDat$treat=="noWinter"]))/mean(log(modDat$total.catch[modDat$treat=="noWinter"]))
...
mayAug.cv=sd(log(modDat$total.catch[modDat$treat=="mayAug"]))/mean(log(modDat$total.catch[modDat$treat=="mayAug"]))
...
wd25.cv=sd(log(modDat$total.catch[modDat$treat=="wd25"]))/mean(log(modDat$total.catch[modDat$treat=="wd25"]))
...
wd50.cv=sd(log(modDat$total.catch[modDat$treat=="wd50"]))/mean(log(modDat$total.catch[modDat$treat=="wd50"]))
...
we50.cv=sd(log(modDat$total.catch[modDat$treat=="we50"]))/mean(log(modDat$total.catch[modDat$treat=="we50"]))
...
nwComp=data.frame(catch.acre=c(modDat$total.catch[modDat$treat=="actual"],rlnorm(n=length(modDat$total.catch[modDat$treat=="actual"]),
                                                                       meanlog = median(pars.nw[,1]),
                                                                       sdlog = median(pars.nw[,2]))),
                  treat=c(rep("actual",length(modDat$total.catch[modDat$treat=="actual"])),rep("model",length(modDat$total.catch[modDat$treat=="actual"]))))

maComp=data.frame(catch.acre=c(modDat$total.catch[modDat$treat=="actual"],rlnorm(n=length(modDat$total.catch[modDat$treat=="actual"]),
                                                                     meanlog = median(pars.ma[,1]),
                                                                     sdlog = median(pars.ma[,2]))),
                  treat=c(rep("actual",length(modDat$total.catch[modDat$treat=="actual"])),rep("model",length(modDat$total.catch[modDat$treat=="actual"]))))

wd25Comp=data.frame(catch.acre=c(modDat$total.catch[modDat$treat=="actual"],rlnorm(n=length(modDat$total.catch[modDat$treat=="actual"]),
                                                                     meanlog = median(pars.wd25[,1]),
                                                                     sdlog = median(pars.wd25[,2]))),
                    treat=c(rep("actual",length(modDat$total.catch[modDat$treat=="actual"])),rep("model",length(modDat$total.catch[modDat$treat=="actual"]))))

wd50Comp=data.frame(catch.acre=c(modDat$total.catch[modDat$treat=="actual"],rlnorm(n=length(modDat$total.catch[modDat$treat=="actual"]),
                                                                     meanlog = median(pars.wd50[,1]),
                                                                     sdlog = median(pars.wd50[,2]))),
                    treat=c(rep("actual",length(modDat$total.catch[modDat$treat=="actual"])),rep("model",length(modDat$total.catch[modDat$treat=="actual"]))))

we50Comp=data.frame(catch.acre=c(modDat$total.catch[modDat$treat=="actual"],rlnorm(n=length(modDat$total.catch[modDat$treat=="actual"]),
                                                                     meanlog = median(pars.we50[,1]),
                                                                     sdlog = median(pars.we50[,2]))),
                    treat=c(rep("actual",length(modDat$total.catch[modDat$treat=="actual"])),rep("model",length(modDat$total.catch[modDat$treat=="actual"]))))
...
The results of these tests suggest that when using coefficient of variation as the variance metric to assess model fit and the $<0.10$ or $>0.90$ rule that the no winter and may-august data reduction models are able to approximate the data, but the 3 percentage reductions are unable to (Table \@ref(tab:bpValueComparison)).
When using standard deviation the p-values suggest all 5 reductions are able to reproduce the actual data (Table \@ref(tab:bpValueComparison)).
However an inspection of the distribution plots suggests that only the seasonal reductions should be trusted (Figure \@ref(fig:catchDataComparisonToActual)).
...
I first started with some basic analyses of the data on a year by year basis to understand how the data changes from year to year and if any lake characteristics explained variation (if there is any) in annual catch per acre estimates across data reduction scenarios for specific creel-years (Figure \@ref(fig:yearAnalysisExampleLakes)).

```{r yearAnalysisExampleLakes, fig.width=8,fig.cap="Comparison of the annual catch per acre estimated from the reduced data for a handful of creel surveys. Most data reductions do not result in annual catch per acre estimates that are much different from the actual data (most are within 1 SD of the catch estimate for that treatment). Colors represent data reduction scenarios, points are mean annual catch per acre estimates across all months of that creel survey and vertical lines represent 1 standard deviation."}
ttrExp=ttrExp%>%
  left_join(cserv[,2:5])%>%
  left_join(lchar[,c(1:3,15)])
...
ggplot(pdat)+theme_classic()+
  ...
  labs(x="Lake-Year",y="Annual Catch per Acre (+/- 1 SD)",color="Scenario")+
  scale_color_viridis_d()

```

```{r year-analysis-diff-actual, fig.cap="Boxplot of the difference between the actual annual catch per acre estimate and the data-reduced annual catch per acre estimate for each creel survey in the creel dataset. Most of the data exhibits differences very near 0 except the seasonal reductions, dots represent outliers (x<|> x's percentile-1.5*interquartile range)"}
...
kable(exceedSummary,
      ...
      caption = "Counts of the number of times the annual catch per acre estimate for the reduced data scenario differs from the actual annual catch per acre estimate for that treatment by more than 1 standard deviation. NAs ocurred where there was no variance in the catch estimate and thus no standard deviation could be calculated for the actual data.")

```

```{r yearAnalysisRecCode, fig.cap="Comparison of the magnitutde of the difference between the mean actual annual catch per acre estimate and the estimate derived from each reduced data set. Vertical lines represent 1 standard deviation. Walleye recruitment codes are along the x axis. Note the y-axis scale, many of the differences are small."}
...
```
```{r yearAnalysisLakeClass,fig.cap="Comparison of the magnitude of the difference between the mean actual annual catch per acre estimate and the estimate derived from each reduced data set. Vertical lines represent 1 standard deviation. Note the y-axis scale, many of the differences are very small."}
lcSum=trLake_chars%>%
...
for(y in 1:length(loopY)){
  ...
  actual.cv=sd(log(tdat$total.catch[tdat$treat=="actual"]))/mean(log(tdat$total.catch[tdat$treat=="actual"]))
  ...
  noWinter.cv=sd(log(tdat$total.catch[tdat$treat=="noWinter"]))/mean(log(tdat$total.catch[tdat$treat=="noWinter"]))
  ...
  mayAug.cv=sd(log(tdat$total.catch[tdat$treat=="mayAug"]))/mean(log(tdat$total.catch[tdat$treat=="mayAug"]))
  ...
  wd25.cv=sd(log(tdat$total.catch[tdat$treat=="wd25"]))/mean(log(tdat$total.catch[tdat$treat=="wd25"]))
  ...
  wd50.cv=sd(log(tdat$total.catch[tdat$treat=="wd50"]))/mean(log(tdat$total.catch[tdat$treat=="wd50"]))
  ...
  we50.cv=sd(log(tdat$total.catch[tdat$treat=="we50"]))/mean(log(tdat$total.catch[tdat$treat=="we50"]))
...
#saveRDS(yearLoopOutput, file = "yearLoopOutput_catch_4.4.25.RData")
...
yearLoopOutput=readRDS("yearLoopOutput_catch_4.4.25.RData")
...
ggplot(ttrExp)+theme_classic()+
  ...
  labs(y="Density",x="Log(Annual Catch per Acre)", fill="Scenario")
```
...
```
The results of these creel-year specific analyses suggest that even on an individual creel year level, the estimated annual catch per acre from the data-reduced scenario generally is not significantly different from the annual catch per acres calculated from the full data set (Figure \@ref(fig:yearLoopComparisonToActual)).
Compared to the same analysis for the full population of creel data all at once instead of on a year-by-year basis, the estimated catch from the reduced data here don't match the actual quite as well for the seasonal data removals, but the majority of years still fall within the typical bounds of acceptance for this test. All models do appear to  fit well and converge (Figure \@ref(fig:yearByyearConvergence)).

There did not appear to be any obvious characteristics of the lakes themselves or their walleye populations that correlated with the magnitude of the difference between the actual estimated annual catch per acre and the reduced-data estimation.
...
This may be in large part due to the relatively small differences between each estimate of annual catch per acre and the reduced-data estimate that could make it hard to see an effects, which would further point towards there being minimal effect of data reductions on annual catch per acre estimate even at the individual creel-year level.
...
1.  Accuracy measure. In order to compare the catch estimates from the reduced data to the actual data I used 1 standard deviation as my measure. This means that a reduced-data catch estimate that is within 1 SD of the actual data catch estimate was considered to be 'the same'. There may be better metrics to use here, and different people may have different opinions about the best metric to use. My rationale for using this metric has been outlined above.
...
set.seed(10)
aComp.lnorm=data.frame(catch.acre=c(ttrExp$total.catch[ttrExp$treat=="actual"],rlnorm(n=length(ttrExp$total.catch[ttrExp$treat=="actual"]),
...
aComp.gamma=data.frame(catch.acre=c(ttrExp$total.catch[ttrExp$treat=="actual"],rgamma(n=length(ttrExp$total.catch[ttrExp$treat=="actual"]),
                                                                    shape = median(pars.a.gamma[,1]),
                                                                    rate = median(pars.a.gamma[,2]))),
                  treat=c(rep("observed",length(ttrExp$total.catch[ttrExp$treat=="actual"])),rep("pred",length(ttrExp$total.catch[ttrExp$treat=="actual"]))))

ln=ggplot(aComp.lnorm)+theme_classic()+
  geom_density(aes(x=catch.acre, fill=treat),alpha=0.3)+
  scale_fill_viridis_d()+labs(x="Annual Catch per Acre",title = "Lognormal")+
  theme(legend.position = c(0.75,0.75))
gma=ggplot(aComp.gamma)+theme_classic()+
  geom_density(aes(x=catch.acre, fill=treat),alpha=0.3)+
  scale_fill_viridis_d()+labs(x="Annual Catch per Acre",title = "Gamma")+
  theme(legend.position = c(0.75,0.75))
ggarrange(ln,gma)
...
aComp.gamma=data.frame(catch.acre=c(ttrExp$total.catch[ttrExp$treat=="actual"],rgamma(n=length(ttrExp$total.catch[ttrExp$treat=="actual"]),
                                                                    ...
                                                                    rate = median(pars.a.gamma[,2]))),
                  treat=c(rep("observed",length(ttrExp$total.catch[ttrExp$treat=="actual"])),rep("pred",length(ttrExp$total.catch[ttrExp$treat=="actual"]))))
nwComp.gamma=data.frame(catch.acre=c(ttrExp$total.catch[ttrExp$treat=="noWinter"],rgamma(n=length(ttrExp$total.catch[ttrExp$treat=="noWinter"]),
...
maComp.gamma=data.frame(catch.acre=c(ttrExp$total.catch[ttrExp$treat=="mayAug"],rgamma(n=length(ttrExp$total.catch[ttrExp$treat=="mayAug"]),
...
wd25Comp.gamma=data.frame(catch.acre=c(ttrExp$total.catch[ttrExp$treat=="wd25"],rgamma(n=length(ttrExp$total.catch[ttrExp$treat=="wd25"]),
...
wd50Comp.gamma=data.frame(catch.acre=c(ttrExp$total.catch[ttrExp$treat=="wd50"],rgamma(n=length(ttrExp$total.catch[ttrExp$treat=="wd50"]),
...
we50Comp.gamma=data.frame(catch.acre=c(ttrExp$total.catch[ttrExp$treat=="we50"],rgamma(n=length(ttrExp$total.catch[ttrExp$treat=="we50"]),
...
a.p.gamma=ggplot(aComp.gamma)+theme_classic()+
  geom_density(aes(x=catch.acre,fill=treat),alpha=0.2)+coord_cartesian(xlim = c(0,250))+
  scale_fill_viridis_d()+labs(x="Annual Catch per Acre", y="Density", title = "Actual", fill=element_blank())
nw.p.gamma=ggplot(nwComp.gamma)+theme_classic()+
  geom_density(aes(x=catch.acre,fill=treat),alpha=0.2)+coord_cartesian(xlim = c(0,250))+
  scale_fill_viridis_d()+labs(x="Annual Catch per Acre", y="Density", title = "No Winter Data \n(April - October)", fill=element_blank())
ma.p.gamma=ggplot(maComp.gamma)+theme_classic()+
  geom_density(aes(x=catch.acre,fill=treat),alpha=0.2)+coord_cartesian(xlim = c(0,250))+
  scale_fill_viridis_d()+labs(x="Annual Catch per Acre", y="Density", title = "May - August \nData Only", fill=element_blank())
wd25.p.gamma=ggplot(wd25Comp.gamma)+theme_classic()+
  geom_density(aes(x=catch.acre,fill=treat),alpha=0.2)+coord_cartesian(xlim = c(0,250))+
  scale_fill_viridis_d()+labs(x="Annual Catch per Acre", y="Density", title = "25% of Weekday \nCreel Visits/Month \nRemoved", fill=element_blank())
wd50.p.gamma=ggplot(wd25Comp.gamma)+theme_classic()+
  geom_density(aes(x=catch.acre,fill=treat),alpha=0.2)+coord_cartesian(xlim = c(0,250))+
  scale_fill_viridis_d()+labs(x="Annual Catch per Acre", y="Density", title = "50% of Weekday \nCreel Visits/Month \nRemoved", fill=element_blank())
we50.p.gamma=ggplot(we50Comp.gamma)+theme_classic()+
  geom_density(aes(x=catch.acre,fill=treat),alpha=0.2)+coord_cartesian(xlim = c(0,250))+
  scale_fill_viridis_d()+labs(x="Annual Catch per Acre", y="Density", title = "50% of Weekdend \nCreel Visits/Month \nRemoved", fill=element_blank())
...
gelmanDiagnostics(catch.actual.gamma) # 1.02 converged
gelmanDiagnostics(catch.nw.gamma) # 1.01 converged
gelmanDiagnostics(catch.ma.gamma) # 1.01 converged
gelmanDiagnostics(catch.25wd.gamma) # 1.01 converged
gelmanDiagnostics(catch.50wd.gamma) # 1.01 converged
gelmanDiagnostics(catch.50we.gamma) # 1.03 converged
...
actual.cv=sd(ttrExp$total.catch[ttrExp$treat=="actual"])/mean(ttrExp$total.catch[ttrExp$treat=="actual"])
...
noWinter.cv=sd(ttrExp$total.catch[ttrExp$treat=="noWinter"])/mean(ttrExp$total.catch[ttrExp$treat=="noWinter"])
...
mayAug.cv=sd(ttrExp$total.catch[ttrExp$treat=="mayAug"])/mean(ttrExp$total.catch[ttrExp$treat=="mayAug"])
...
wd25.cv=sd(ttrExp$total.catch[ttrExp$treat=="wd25"])/mean(ttrExp$total.catch[ttrExp$treat=="wd25"])
...
wd50.cv=sd(ttrExp$total.catch[ttrExp$treat=="wd50"])/mean(ttrExp$total.catch[ttrExp$treat=="wd50"])
...
we50.cv=sd(ttrExp$total.catch[ttrExp$treat=="we50"])/mean(ttrExp$total.catch[ttrExp$treat=="we50"])
